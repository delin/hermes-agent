...
class TestHandleMaxIterations:
    ...
    def test_codex_summary_uses_summary_relay_identity(self, agent):
        agent.api_mode = "codex_responses"
        agent.provider = "openai-codex"
        agent.model = "gpt-5-codex"
        agent.base_url = "https://chatgpt.com/backend-api/codex"
        agent._cached_system_prompt = "You are helpful."
        agent._current_api_request_id = "turn-1:api:60"
        relay_calls = []
        ...
        client = SimpleNamespace(
            responses=SimpleNamespace(create=lambda **kwargs: EventStream())
        )

        def relay_stream(request, stream_factory, **kwargs):
            relay_calls.append(kwargs)
        ...
        with (
            ...
            patch("agent.relay_llm.stream", side_effect=relay_stream),
            patch("agent.relay_llm.complete_logical_call") as complete_logical,
        ):
            result = agent._handle_max_iterations(
                [{"role": "user", "content": "do stuff"}],
        ...
        metadata = relay_calls[0]["metadata"]
        assert metadata["api_request_id"].startswith("iteration-summary:")
        assert metadata["api_request_id"] != "turn-1:api:60"
        assert metadata["call_role"] == "iteration_summary"
        complete_logical.assert_called_once_with(
    ...
    def test_codex_summary_sanitizes_orphan_tool_results(self, agent):
        ...
        def fake_run_codex_stream(kwargs, **_private_kwargs):
