...
def _bind_task_fence_worker(target: Callable[[], Any]) -> Callable[[], Any]:
    """Copy thread context while keeping Task Fence authority unprivileged."""
    ...
    return lambda: context.run(_bound)


class _AnthropicStreamLifecycle:
    """Keep one audited invocation across the lazy Anthropic stream lifecycle."""

    def __init__(self, manager: Any, stream: Any, context: contextvars.Context):
        self._manager = manager
        self._stream = stream
        self._context = context
        self._iterator: Any = None

    def _run(self, callback: Callable[..., Any], *args: Any) -> Any:
        return self._context.copy().run(callback, *args)

    @property
    def response(self) -> Any:
        return self._run(getattr, self._stream, "response", None)

    def __iter__(self):
        self._iterator = self._run(iter, self._stream)
        return self

    def __next__(self):
        if self._iterator is None:
            self.__iter__()
        return self._run(next, self._iterator)

    def get_final_message(self) -> Any:
        return self._run(self._stream.get_final_message)

    def close(self) -> None:
        close = self._run(getattr, self._stream, "close", None)
        if callable(close):
            self._run(close)

    def __exit__(self, *args: Any) -> Any:
        return self._run(self._manager.__exit__, *args)
            ...
            if agent.api_mode == "codex_responses":
                ...
                if summary_policy is None:
                    summary_response = agent._run_codex_stream(
                        codex_kwargs,
                        _relay_api_request_id=summary_api_request_id,
                        _relay_call_role="iteration_summary",
                    )
                else:
                    summary_response = agent._run_codex_stream(
                        codex_kwargs,
                ...
                def _summary_anthropic_create(request):
                    if summary_policy is None:
                ...
                summary_response = _managed_summary_call(
                    summary_kwargs,
                    lambda request: _create_openai_chat_completion(
                        agent,
                        summary_client,
                        request,
        ...
        if final_response:
            ...
            with _task_fence_iteration_summary_attempt(
                ...
                if agent.api_mode == "codex_responses":
                    ...
                    def _retry_anthropic_create(request):
                        if retry_policy is None:
                            return agent._anthropic_messages_create(request)
                        return agent._anthropic_messages_create(
                            request,
                    ...
                    try:
                        ...
                        if is_streaming_access_denied_error(_bedrock_exc):
                            ...
                            with task_fence_model_handoff(
                                ...
                                fallback_response = client.converse(**final_kwargs)
                            return normalize_converse_response(fallback_response)
                        if is_stale_connection_error(_bedrock_exc):
                            invalidate_runtime_client(region)
                        raise
    ...
    def _call_anthropic(request_client):
        ...
        def _open_anthropic_stream(next_api_kwargs: dict[str, Any]):
            ...
            with task_fence_model_handoff(
                ...
                lifecycle = _AnthropicStreamLifecycle(
                    manager,
                    raw_stream,
                    contextvars.copy_context(),
                )
                _stream_context["manager"] = lifecycle
                return lifecycle
