...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        ...
        if not task_fence_handoff:
            return execute(final_args)

        from tools.registry import _task_fence_tool_handoff

        with (
            bind_causal_envelope(causal_envelope),
            _task_fence_tool_handoff(
                function_name,
                final_args,
                {
                    "task_id": effective_task_id or "",
        ...
        try:
            try:
                ...
                managed = _run_agent_tool_execution_middleware(
                    ...
                    causal_envelope=invocation_envelope,
                    execute=_execute,
                    task_fence_handoff=agent_runtime_owns_post_tool_hook(
                        agent, function_name
                    ),
